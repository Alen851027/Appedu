Shader "KriptoFX/AE/Lightning(randomID)" {
Properties {
	...
	_ClipScale("Clip Scale (xy)", Vector) = (7, 7, 0, 0) 
	...
	SubShader {
		Pass {
			...
			v2f vert (appdata_t v)
			{
				...
				o.color = v.color;
					float time = _Time.x + v.randomID.x * 255 + v.randomID.y*255;
				o.timeLerp = lerp(length(v.velocity), time, _UseVelocity);
			...
			half4 frag (v2f i) : SV_Target
			{
				...
				half alpha = tex2D(_MainTex, i.uvMain * _ClipScale + _Time.x * 5).b;
