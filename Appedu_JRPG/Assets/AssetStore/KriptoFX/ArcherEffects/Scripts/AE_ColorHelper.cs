...
public static class AE_ColorHelper
{
    ...
    public static void ChangeObjectColorByHUE(GameObject go, float hue)
    {
        ...
        foreach (var rend in renderers)
        {
            ...
            UpdateShaderColor(rend.transform, hue);
        }
        ...
        foreach (var skinRend in skinRenderers)
        {
            ...
            UpdateShaderColor(skinRend.transform, hue);
        }
        ...
        foreach (var proj in projectors)
        {
            ...
            UpdateShaderColor(proj.transform, hue);
        }
        ...
        foreach (var light in lights)
        {
            ...
            var lightCurves = light.GetComponent<AE_LightCurves>();
            if (lightCurves != null) lightCurves.SetStartColor(HSVToColor(hsv));
        }
        ...
        foreach (var ps in particles)
        {
            ...
            UpdateShaderColor(ps.transform, hue);
        }

        var AE_shaderColorGradients = go.GetComponentsInChildren<AE_ShaderColorGradient>(true);
        
        foreach (var AE_shaderColorGradient in AE_shaderColorGradients)
        {
            AE_shaderColorGradient.HUE = hue;
        }

        
    }
    
    static void UpdateShaderColor(Transform tr, float hue)
    {
        var colorCurves = tr.GetComponent<AE_ShaderColorGradient>();
        if (colorCurves != null)
        {
            var hsv = ColorToHSV(Color.red);
            hsv.H = hue;
            colorCurves.SetStartColor(HSVToColor(hsv));
        }
    }
